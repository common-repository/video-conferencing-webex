...
define( 'VCW_LOG_DIR', $upload_dir['basedir'] . '/vcw-logs/' );
...
class Logger {
	...
	public function create_dir() {
		if ( ! is_dir( VCW_LOG_DIR ) ) {
			//Create our directory if it does not exist
			mkdir( VCW_LOG_DIR );
			file_put_contents( VCW_LOG_DIR . '/' . '.htaccess', 'deny from all' );
			file_put_contents( VCW_LOG_DIR . '/' . 'index.html', '' );
		}

		//Set Log directory Path
		$this->log_dir = VCW_LOG_DIR;
	}
	...
	public static function get_log_files() {
		$files  = @scandir( VCW_LOG_DIR, SCANDIR_SORT_DESCENDING );
		$result = array();
	...
	public static function remove( $handle ) {
		...
		if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
			$file = realpath( trailingslashit( VCW_LOG_DIR ) . $logs[ $handle ] );
			if ( 0 === stripos( $file, realpath( trailingslashit( VCW_LOG_DIR ) ) ) && is_file( $file ) ) {
				$removed = unlink( $file );
			}
		}
